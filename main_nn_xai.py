...
mpc = tf.keras.models.load_model('models/nn_controller_xai.h5')
...
for start_index in test_index_start:
    ...
    np.save(f'./results/state_trajectories/nn_xai/start_index_{start_index}', x_res)

results = np.vstack([avg_violations, max_violations, total_grid_energy])
results = np.array(results)
np.save('results/results_nn_xai', results)
