...
for start_index in test_index_start:
    ...
    for k in range(start_index, start_index+7*24):
        ...
        input_test_scaled.append(np.hstack([x0_scaled, T_s[k].reshape(1, -1), SR_s[k].reshape(1, -1)]))
        # for mini controller:
    ...
    input_test_scaled = np.array(input_test_scaled)
    np.save(f'data/test_pca/{start_index}', input_test_scaled)

    np.save(f'./results/state_trajectories/nn/start_index_{start_index}', x_res)

results = np.vstack([avg_violations, max_violations, total_grid_energy])
results = np.array(results)
np.save('results/results_nn', results)
